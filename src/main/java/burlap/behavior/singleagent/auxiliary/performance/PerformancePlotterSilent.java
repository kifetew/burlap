 ...
 * <p>
 * The way this class should be used is the constructor is first called. Then the startGUI() method. A trial indicates a single evaluation of
 ...
 *
 */
public class PerformancePlotterSilent //extends JFrame 
implements EnvironmentObserver {
...
//	protected AgentDatasets	curAgentDatasets;
	 ...
	 */
	public PerformancePlotterSilent(String firstAgentName, int chartWidth, int chartHeight, int columns, int maxWindowHeight,
								TrialMode trialMode, boolean displayPlots, PerformanceMetric...metrics){

		this.curAgentName = firstAgentName;
		
		this.agentTrials = new HashMap<String, List<Trial>>();
		this.agentTrials.put(this.curAgentName, new ArrayList<PerformancePlotterSilent.Trial>());
	...
	synchronized public void startNewAgent(final String agentName){
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				synchronized (PerformancePlotterSilent.this) {
				
					PerformancePlotterSilent.this.endTrialsForCurrentAgent();
					
					
					PerformancePlotterSilent.this.curAgentName = agentName;
					PerformancePlotterSilent.this.agentTrials.put(PerformancePlotterSilent.this.curAgentName, new ArrayList<PerformancePlotterSilent.Trial>());
//					PerformancePlotterSilent.this.curAgentDatasets = new AgentDatasets(curAgentName);
	...
	protected void launchThread(){
		 Thread refreshThread = new Thread(new Runnable() {
				...
				public void run() {
					while(true){
						PerformancePlotterSilent.this.updateTimeSeries();
						try {
							Thread.sleep(PerformancePlotterSilent.this.delay);
						} catch (InterruptedException e) {
							e.printStackTrace();
						}
					}
	...
	synchronized protected void updateTimeSeries(){
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				if(PerformancePlotterSilent.this.trialMode.mostRecentTrialEnabled()){
					synchronized (PerformancePlotterSilent.this) {
						
						synchronized (PerformancePlotterSilent.this.trialUpdateComplete) {
							
							if(PerformancePlotterSilent.this.needsClearing){
//								PerformancePlotterSilent.this.curAgentDatasets.clearNonAverages();
								PerformancePlotterSilent.this.needsClearing = false;
							}
							
							if(PerformancePlotterSilent.this.curTimeStep > PerformancePlotterSilent.this.lastTimeStepUpdate){
//								PerformancePlotterSilent.this.updateCSRSeries();
								PerformancePlotterSilent.this.lastTimeStepUpdate = curTimeStep;
							}
							if(PerformancePlotterSilent.this.curEpisode > PerformancePlotterSilent.this.lastEpisode){
//								PerformancePlotterSilent.this.updateCERSeries();
//								PerformancePlotterSilent.this.updateAERSeris();
//								PerformancePlotterSilent.this.updateMERSeris();
//								PerformancePlotterSilent.this.updateCSESeries();
//								PerformancePlotterSilent.this.updateSESeries();
								
								PerformancePlotterSilent.this.lastEpisode = PerformancePlotterSilent.this.curEpisode;
							}
							
							
							PerformancePlotterSilent.this.trialUpdateComplete.b = true;
							PerformancePlotterSilent.this.trialUpdateComplete.notifyAll();
	...
	protected void endTrialsForCurrentAgent(){
		...
		List<Trial> trials = PerformancePlotterSilent.this.agentTrials.get(aName);
		int [] n = PerformancePlotterSilent.this.minStepAndEpisodes(trials);
